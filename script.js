...
const d = new Date()
const den = d.getDate() - 1
const mesic= d.getMonth()
const svatek = mesice[mesic][den]

const vysledek = document.createElement("p")
vysledek.innerHTML = `Dneska má svátek <strong>${svatek}</strong>.`
document.body.appendChild(vysledek)



// 2. příklad - Dnešní datum
const ceskeDatum = (datum) => {
    let mesice = ["ledna", "února", "března", "dubna", "května", "června", "července", "srpna", "září", "října", "listopadu", "prosince"]

    return `${datum.getDate()}. ${mesice[datum.getMonth()]} ${datum.getFullYear()}`
}

const datum = new Date()
const dnesniDen = document.createElement("p")
dnesniDen.innerHTML = `Dnešní datum je: <strong>${ceskeDatum(datum)}</strong>`
document.body.appendChild(dnesniDen)